...
checkpoint_dir = f"/mnt/czi-sci-ai/imaging-models/kyle/experiments/cryolens_mlchallenge/train_mlc_curriculum_{experiment_id}"
...
class CurriculumDataLoader:
    def __init__(self, dataset, batch_size, max_epochs):
        self.dataset = dataset
        self.batch_size = batch_size
        self.max_epochs = max_epochs

    def get_dataloader(self, epoch):
        weights = self.dataset.sample_weights(epoch, self.max_epochs)
        sampler = WeightedRandomSampler(weights, num_samples=len(weights), replacement=True)
        return DataLoader(self.dataset, batch_size=self.batch_size, sampler=sampler, num_workers=0)

class CurriculumLearningCallback(Callback):
    def __init__(self, dataset, max_epochs, easy_ratio=0.8):
        """
        Args:
            dataset: The dataset to adjust sampling weights for.
            max_epochs: The total number of training epochs.
            easy_ratio: Initial ratio of easy samples (default is 80% easy samples).
        """
        super().__init__()
        self.dataset = dataset
        self.max_epochs = max_epochs
    ...
    def on_train_epoch_end(self, trainer, pl_module):
        """
        Called at the end of every training epoch to update difficulty scores and sampling weights.
        """
        epoch = trainer.current_epoch
        # Adjust sample weights dynamically
        progress = epoch / self.max_epochs
        weights = self.dataset.sample_weights(epoch, self.max_epochs, easy_ratio=self.easy_ratio)
        
        # Update sampler with new weights
        sampler = WeightedRandomSampler(weights, num_samples=len(weights), replacement=True)
        trainer.train_dataloader.sampler = sampler

        print(f"Updated sampling weights for epoch {epoch + 1}/{self.max_epochs}")

class DifficultyScoreCallback(Callback):
    def __init__(self, dataset, easy_ratio=0.8):
        """
        Args:
            dataset: The dataset to update difficulty scores for.
            easy_ratio: Initial ratio of easy samples.
        """
        super().__init__()
        self.dataset = dataset
        self.easy_ratio = easy_ratio
    ...
    def on_train_batch_end(self, trainer, pl_module, outputs, batch, batch_idx):
        """
        Update difficulty scores after every training batch.
        """
        # Extract per-sample losses
        r_loss_per_sample = outputs["reconstruction_loss"].numpy().flatten()
        s_loss_per_sample = outputs["similarity_loss"].numpy().flatten()

        # Ensure the losses are 1D arrays (batch_size,)
        if len(r_loss_per_sample.shape) > 1 or len(s_loss_per_sample.shape) > 1:
            raise ValueError("Loss per sample must be 1D (batch_size).")

        if r_loss_per_sample.shape[0] != s_loss_per_sample.shape[0]:
            raise ValueError(f"r_loss_per_shape {r_loss_per_sample.shape} s_loss_per_sample {s_loss_per_sample.shape}.")

        # Calculate difficulty scores
        difficulty_scores = r_loss_per_sample + s_loss_per_sample  # (batch_size,)
        batch_indices = range(
            batch_idx * trainer.train_dataloader.batch_size,
            (batch_idx + 1) * trainer.train_dataloader.batch_size,
        )

        # Update dataset difficulty scores
        for idx, score in zip(batch_indices, difficulty_scores):
            if idx < len(self.dataset.difficulty_scores):  # Avoid out-of-bounds
                self.dataset.difficulty_scores[idx] = score
...
class VAEModel(LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        r_loss_per_sample = torch.mean((output - img) ** 2, dim=(1, 2, 3))  # (batch_size,)
        kld_per_sample = -0.5 * torch.sum(1 + log_var - mu ** 2 - log_var.exp(), dim=1)  # (batch_size,)
        s_loss_per_sample = self.gamma * self.similarity_loss(mol_id, mu, per_sample=True)  # (batch_size,)

        print(f"r_loss_per_sample: {r_loss_per_sample.shape}, s_loss_per_sample: {s_loss_per_sample.shape}")


        # Aggregate batch losses
        r_loss = torch.mean(r_loss_per_sample)
        kld = self.beta * self.kld_weight * torch.mean(kld_per_sample)  # Average for batch
        s_loss = torch.mean(s_loss_per_sample)
        ...
        # Return per-sample losses as 1D arrays for callback
        return {
            "loss": loss,
            "reconstruction_loss": r_loss_per_sample.cpu().detach(),
            "similarity_loss": s_loss_per_sample.cpu().detach(),
        }
...
dataloader = DataLoader(copick_dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=0)
...
# Set up the dataset
copick_dataset.difficulty_scores = np.ones(len(copick_dataset), dtype=np.float32) * 0.5  # Initialize scores uniformly

# Curriculum Learning Callback
curriculum_callback = CurriculumLearningCallback(
    dataset=copick_dataset,
    max_epochs=EPOCHS,
    easy_ratio=0.8
)

difficulty_callback = DifficultyScoreCallback(dataset=copick_dataset)

# Trainer with curriculum learning
trainer = Trainer(
    max_epochs=EPOCHS,
    callbacks=[checkpoint_callback, curriculum_callback, difficulty_callback],
